...
// ✨ CORREÇÃO 1: Adicionadas as funções extras de busca (query, orderBy, limit, getDocs)
...
document.getElementById('lead-form').addEventListener('submit', async (e) => {
    ...
    try {
        // ✨ CORREÇÃO 2: Busca o último código gerado antes de salvar
        ...
        // Empacota todos os dados do HTML (Agora com o código)
        const data = {
            code: proximoCodigo, // ✨ Injetando o código aqui
...
window.enviarLeadZap = async (e) => {
    ...
    try {
        // ✨ CORREÇÃO 3: Busca o último código gerado para o WIDGET também!
        const leadsRef = collection(db, 'leads');
        const q = query(leadsRef, orderBy('code', 'desc'), limit(1));
        const snap = await getDocs(q);
        ...
        await addDoc(leadsRef, {
            code: proximoCodigo, // ✨ Injetando o código aqui no widget
            nome: nome,
            nomeLoja: loja,
            email: email,
            whatsapp: wpp,
